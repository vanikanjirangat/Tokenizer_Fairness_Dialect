...
from prompts import get_di_data_for_ft
...
def main(args):
    _, test_dataset = get_di_data_for_ft(mode="inference")
    experiment = args.experiment
    peft_model_id = f"{experiment}/checkpoint-165"    
    ...
    y_true=labels
    diff=list((set(results) - set(y_true)))
    print("predicted label",list(set(results)))
    print("true labels",list(set(y_true)))
    print("Difference",diff)
    print("______________________________________________")
    for i,k in enumerate(results):
        if k not in labs:
            print("(Label predicted OOL, Corresponding True label)",(k,y_true[i]))
    print(classification_report(y_true,results,labels=labs))
    print(confusion_matrix(y_true,results,labels=labs))
