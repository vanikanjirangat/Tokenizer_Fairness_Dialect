...
#csv_df = pd.read_csv("metrics_output_llama3.csv")
...
csv_df["lang"] = csv_df["lang_orig"].map(flores_to_lang)
...
print(unmapped[["lang_orig"]].drop_duplicates())
...
#print(final_df)
md=final_df[["lang","ip_mean","lang_orig","macro_f1"]]
print(md)
# 7. Export to CSV
#final_df.to_csv("merged_lang_metrics_Phi_mini.csv", index=False)
final_df.to_csv("merged_lang_metrics_llama.csv", index=False)
print("Merged data saved to 'merged_lang_metrics.csv'")
correlation = final_df["ip_mean"].corr(final_df["micro_f1"])
print(f"Pearson correlation between IP mean and micro_f1: {correlation:.4f}")
cor_macro=final_df["ip_mean"].corr(final_df["macro_f1"])
print(f"Pearson correlation between IP mean and macro_f1: {correlation:.4f}")
...
def classify(row):
    lang_name = row['lang_orig']
    lang_code = row['lang_orig']
